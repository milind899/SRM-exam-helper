      ...
      configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
          ...
          if (req.url?.startsWith('/api/')) {
            console.log('API Middleware hit:', req.method, req.url);
            ...
            if (req.method === 'POST') {
              if (req.url.includes('/challenge/create')) {
                try {
                  const data = await parseBody();
                  const sql = await getSql();
                  if (!sql) return sendJson({ error: 'Database configuration missing' }, 500);
                  ...
                  if (!questions || questions.length === 0) return sendJson({ error: 'No questions found' }, 404);
              ...
              if (req.url.includes('/submit_test')) {
                try {
                  const data = await parseBody();
                  const sql = await getSql();
